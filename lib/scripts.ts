...
export const claimTx = async (
    ...
    const txId = await program.methods
        .claim(new anchor.BN(icoData.data.seed), new anchor.BN(purchaseData.data.seed))
        .accounts({
            buyer,
            // @ts-ignore
            icoPot: icoPot,
...
export const findAllICOs = async (
    ...
    const icoAccs = await program.provider.connection.getProgramAccounts(program.programId, {
        filters,
        encoding: 'base64',
    });

    return icoAccs.map((icoAcc) => {
        let data: any = program.coder.accounts.decode<IcoState>('icoState', icoAcc.account.data);
...
export const findPurchasesOld = async (
    ...
    const purchaseAccs = await program.provider.connection.getProgramAccounts(program.programId, {
        filters,
        encoding: 'base64',
    });

    return purchaseAccs.map((purchaseAcc) => {
        let data: any = program.coder.accounts.decode<UserPurchase>('userPurchase', purchaseAcc.account.data);
        data.seed = data.seed.toNumber();
    ...
    const filtered = allPurchases
        ...
        .map(({ publicKey, account }) => ({
            ...
            data: {
                ...
                refCode: account.refCode !== '' ? account.refCode : null,
            },
        }));
...
export const getUnlocked = (data: UserPurchase, ico: IcoState) => {
    ...
    let intervals = Math.floor((now - cliffFinish) / ico.vestingInterval.toNumber()) + 1;
...
export const getValue = (ico: IcoState, amount: anchor.BN) => {
    ...
    if (ico.endPrice.eq(new anchor.BN(0))) {
        ...
        let currentPrice = ico.endPrice.sub(ico.startPrice).mul(ico.totalSold).div(ico.amount).add(ico.startPrice);
        let executedPrice = ico.endPrice
            .sub(ico.startPrice)
            .mul(ico.totalSold.add(availableAmount))
            .div(ico.amount)
            .add(ico.startPrice);
        let value = availableAmount.mul(currentPrice.add(executedPrice)).div(ico.icoDecimals.mul(new anchor.BN(2)));
        return { availableAmount, value };
    }
};
