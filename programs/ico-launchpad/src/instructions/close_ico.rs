...
impl CloseIco<'_> {
    pub fn process_instruction(ctx: &mut Context<Self>, seed: u64) -> Result<()> {
        ...
        // check ico authority or admin
        require!(
            ico_pot.owner.eq(&ctx.accounts.authority.key()),
            LaunchpadError::InvalidIcoOwner
        );

        require!(ico_pot.is_closed == 0, LaunchpadError::AlreadyClosed);

        // refund remained ico tokens
        let inner_seeds = [
        ...
        transfer_checked(
            ...
            ico_pot.amount - ico_pot.total_sold + ico_pot.bonus_reserve,
            ctx.accounts.ico_mint.decimals,
        )?;

        ico_pot.amount = ico_pot.total_sold;
        ico_pot.bonus_reserve = 0;
        ico_pot.is_closed = 1;
